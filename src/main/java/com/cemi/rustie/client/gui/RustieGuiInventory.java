...
public class RustieGuiInventory extends GuiContainer implements IRecipeShownListener {
	protected static final ResourceLocation BUTTON_TEXTURES = new ResourceLocation(Rustie.MODID,
	...
	public void initGui() {
		...
		this.recipeBookGui.func_194303_a(this.width, this.height, this.mc, this.widthTooNarrow,
				((ContainerPlayer) this.inventorySlots).craftMatrix);

		// this.guiLeft = this.recipeBookGui.updateScreenPosition(this.widthTooNarrow,
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		// this.fontRenderer.drawString(I18n.format("container.crafting"), 97, 8,
		// 4210752);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		drawEntityOnScreen(this.width / 6, this.height * 3 / 4, height / 3, 51 - this.oldMouseX, 25 - this.oldMouseY,
				mousePressed, this.width, this.height, this.mc.player);
		Gui.drawRect(0, 0, this.width, this.height, 0x05A8A8A8);
		for (int i = 5; i < 45; ++i) {
			if (i < 9) {
				Gui.drawRect(width * 1 / 6 + i % 5 * (int) (itemXSize + (itemXSize / 8)) - (int) (itemXSize * 1.1f),
						height * 8 / 10, width * 1 / 6 + i % 5 * (int) (itemXSize + (itemXSize / 8))
								- (int) (itemXSize * 1.1f) - (int) itemXSize,
						height * 8 / 10 + (int) itemYSize, 0x1FFFFFFF);
			} else if (i > 35) {
				Gui.drawRect(width * 4 / 10 + i % 9 * (int) (itemXSize + (itemXSize / 8)) - (int) (itemXSize * 2),
						height * 1 / 2 + i / 9 * (int) (itemXSize + (itemXSize / 8)) + 8 + (int) itemYSize,
						width * 4 / 10 + i % 9 * (int) (itemXSize + (itemXSize / 8)) - (int) (itemXSize),
						height * 1 / 2 + i / 9 * (int) (itemXSize + (itemXSize / 8)) + 8 + (int) (itemYSize * 2),
						0x1FFFFFFF);
			} else {
				Gui.drawRect(width * 4 / 10 + i % 9 * (int) (itemXSize + (itemXSize / 8)) - (int) (itemXSize * 2),
						height * 1 / 2 + i / 9 * (int) (itemXSize + (itemXSize / 8)) + (int) itemYSize,
						width * 4 / 10 + i % 9 * (int) (itemXSize + (itemXSize / 8)) - (int) (itemXSize),
						height * 1 / 2 + i / 9 * (int) (itemXSize + (itemXSize / 8)) + (int) (itemYSize * 2),
						0x1FFFFFFF);
			}
		}
		...
		for (int i = 0; i < 9; i++) {
			...
			this.drawGradientRect(x, y, rightX, downY, 0x00FFFFFF, 0x1FFFFFFF);
			GlStateManager.pushMatrix();
			GlStateManager.scale(scale, scale, 1f);
			...
			this.drawString(this.fontRenderer, "0", statsX, statsY, 0xFFFFFFFF);
			GlStateManager.popMatrix();
			// Draw images
		}
	}
	...
	public static void drawEntityOnScreen(int posX, int posY, int scale, float mouseX, float mouseY,
			boolean mousePressed, int width, int height, EntityLivingBase ent) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		try {
			...
			Field returningStackDestSlotField = this.getClass().getSuperclass()
					.getDeclaredField("returningStackDestSlot");
			...
			if (!itemstack.isEmpty()) {
				...
				if (itemstack.getItem() instanceof ItemBase)
					if (itemstack.hasTagCompound())
						s = itemstack.getTagCompound().getInteger("rustieCount") > 1
								? "x" + itemstack.getTagCompound().getInteger("rustieCount")
								: null;
					else
						s = itemstack.getCount() > 1 ? "x" + itemstack.getCount() : null;
				if (!draggedItemStack.isEmpty() && (boolean) rightClickField.get(this)) {
					itemstack = itemstack.copy();
					if (itemstack.getItem() instanceof ItemBase) {
						if (itemstack.hasTagCompound())
							itemstack.getTagCompound().setInteger("rustieCount", MathHelper
									.ceil((float) itemstack.getTagCompound().getInteger("rustieCount") / 2.0F));
					} else {
			...
			if (!returningStack.isEmpty()) {
				float f = (float) ((long) (Minecraft.getSystemTime() - (long) returningStackTimeField.get(this))
						/ 100.0F);
	...
	private void drawSlot(Slot slotIn) {
		...
		try {
			...
			boolean flag = false;
			boolean flag1 = slotIn == (Slot) clickedSlotField.get(this)
					&& !((ItemStack) draggedField.get(this)).isEmpty() && !(boolean) rightClickField.get(this);
			...
			if (slotIn == (Slot) clickedSlotField.get(this) && !((ItemStack) draggedField.get(this)).isEmpty()
					&& (boolean) rightClickField.get(this) && !itemstack.isEmpty()) {
				itemstack = itemstack.copy();
				...
				if (Container.canAddItemToSlot(slotIn, itemstack1, true)
					...
					flag = true;
					Container.computeStackSize(this.dragSplittingSlots, (int) dragSplittingLimitField.get(this),
							itemstack, slotIn.getStack().isEmpty() ? 0 : slotIn.getStack().getCount());
			...
			if (!flag1) {
				...
				GlStateManager.popMatrix();
				this.itemRender.renderItemOverlayIntoGUI(this.fontRenderer, itemstack, i + (int) (itemXSize / 2),
						j + (int) (itemYSize / 2), s);
			}
			...
			if (!itemstack.isEmpty() && this.dragSplitting) {
				if ((int) dragSplittingLimitField.get(this) == 2) {
					...
					for (Slot slot : this.dragSplittingSlots) {
						...
						dragSplittingRemnantField.set(this,
								(int) dragSplittingRemnantField.get(this) - itemstack1.getCount() - i);
					}
				}
			}
		} catch (Exception e) {
	...
	private void drawItemStack(ItemStack stack, int x, int y, String altText) {
		try {
			...
			GlStateManager.popMatrix();
			this.itemRender.renderItemOverlayIntoGUI(font, stack, x + (int) (itemXSize / 2),
					y + (int) (itemYSize / 2) /*
												 * - (((ItemStack)draggedStackField.get(this)).isEmpty() ? 0 :
												 * (int)(itemYSize / 2))
												 */, altText);
			this.zLevel = 0.0F;
			this.itemRender.zLevel = 0.0F;
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		try {
			...
			doubleClickField.set(this,
					(Slot) lastClickSlotField.get(this) == slot && i - (long) lastClickTimeField.get(this) < 250L
							&& (int) lastClickButtonField.get(this) == mouseButton);
					...
					if (this.mc.gameSettings.touchscreen) {
						...
						if (this.mc.player.inventory.getItemStack().isEmpty()) {
							if (this.mc.gameSettings.keyBindPickBlock.isActiveAndMatches(mouseButton - 100)) {
								...
								if (flag2) {
									shiftClickedSlotField.set(this,
											slot != null && slot.getHasStack() ? slot.getStack().copy()
													: ItemStack.EMPTY);
	...
	protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
		try {
			...
			Field draggedStackField = this.getClass().getSuperclass().getDeclaredField("draggedStack");
			draggedStackField.setAccessible(true);
			Field currentDragTargetSlotField = this.getClass().getSuperclass()
					.getDeclaredField("currentDragTargetSlot");
			...
			if (clickedSlotField.get(this) != null && this.mc.gameSettings.touchscreen) {
				if (clickedMouseButton == 0 || clickedMouseButton == 1) {
					if (((ItemStack) draggedStackField.get(this)).isEmpty()) {
						if (slot != (Slot) clickedSlotField.get(this)
								&& !((Slot) clickedSlotField.get(this)).getStack().isEmpty()) {
							draggedStackField.set(this, ((Slot) clickedSlotField.get(this)).getStack().copy());
						}
					} else if (((ItemStack) draggedStackField.get(this)).getCount() > 1 && slot != null
							&& Container.canAddItemToSlot(slot, (ItemStack) draggedStackField.get(this), false)) {
						long i = Minecraft.getSystemTime();

						if (currentDragTargetSlotField.get(this) == slot) {
							if (i - (long) dragItemDropDelayField.get(this) > 500L) {
								this.handleMouseClick((Slot) clickedSlotField.get(this),
										((Slot) clickedSlotField.get(this)).slotNumber, 0, ClickType.PICKUP);
					...
					&& (itemstack.getCount() > this.dragSplittingSlots.size()
							|| (int) dragSplittingLimitField.get(this) == 2)
	...
	protected void mouseReleased(int mouseX, int mouseY, int state) {
		...
		try {
			...
			if ((boolean) doubleClickField.get(this) && slot != null && state == 0
					&& this.inventorySlots.canMergeSlot(ItemStack.EMPTY, slot)) {
				if (isShiftKeyDown()) {
					if (!((ItemStack) shiftClickedSlotField.get(this)).isEmpty()) {
						for (Slot slot2 : this.inventorySlots.inventorySlots) {
							if (slot2 != null && slot2.canTakeStack(this.mc.player) && slot2.getHasStack()
									&& slot2.isSameInventory(slot) && Container.canAddItemToSlot(slot2,
											(ItemStack) shiftClickedSlotField.get(this), true)) {
				...
				if (clickedSlotField.get(this) != null && this.mc.gameSettings.touchscreen) {
					if (state == 0 || state == 1) {
						...
						boolean flag2 = Container.canAddItemToSlot(slot, (ItemStack) draggedStackField.get(this),
								false);

						if (k != -1 && !((ItemStack) draggedStackField.get(this)).isEmpty() && flag2) {
							this.handleMouseClick((Slot) clickedSlotField.get(this),
									((Slot) clickedSlotField.get(this)).slotNumber, state, ClickType.PICKUP);
					...
					this.handleMouseClick((Slot) null, -999,
							Container.getQuickcraftMask(0, (int) dragSplittingLimitField.get(this)),
							ClickType.QUICK_CRAFT);

					for (Slot slot1 : this.dragSplittingSlots) {
						this.handleMouseClick(slot1, slot1.slotNumber,
								Container.getQuickcraftMask(1, (int) dragSplittingLimitField.get(this)),
								ClickType.QUICK_CRAFT);
					}

					this.handleMouseClick((Slot) null, -999,
							Container.getQuickcraftMask(2, (int) dragSplittingLimitField.get(this)),
							ClickType.QUICK_CRAFT);
					...
					if (this.mc.gameSettings.keyBindPickBlock.isActiveAndMatches(state - 100)) {
						...
						if (flag1) {
							shiftClickedSlotField.set(this,
									slot != null && slot.getHasStack() ? slot.getStack().copy() : ItemStack.EMPTY);
						}
	...
	protected boolean hasClickedOutside(int p_193983_1_, int p_193983_2_, int p_193983_3_, int p_193983_4_) {
		...
		return this.recipeBookGui.hasClickedOutside(p_193983_1_, p_193983_2_, this.width * 1 / 4, this.height * 1 / 10,
				this.width / 2, this.height * 8 / 1) && flag;
	}
	...
	protected void handleMouseClick(Slot slotIn, int slotId, int mouseButton, ClickType type) {
		if (mouseButton == 1 && type == ClickType.PICKUP && !slotIn.getStack().isEmpty())
			if (slotIn.getStack().getItem() instanceof ItemBase) {
				...
				// CPacketClickWindow(mc.player.inventoryContainer.windowId, slotId,
				// mouseButton, type, ItemStack.EMPTY,
	...
	protected void renderHoveredToolTip(int p_191948_1_, int p_191948_2_) {
		try {
			...
			if (this.mc.player.inventory.getItemStack().isEmpty() && (Slot) hoveredSlotField.get(this) != null
					&& super.getSlotUnderMouse().getHasStack()) {
	...
	protected void renderCustomToolTip(ItemStack stack) {
		// Panel
		Gui.drawRect(width * 4 / 10 - (int) (itemXSize * 2), height / 15 + (int) itemYSize,
				width * 4 / 10 + 9 * (int) (itemXSize + (itemXSize / 8)) - (int) (itemXSize * 2), height * 1 / 2,
				0x01FFFFFF);

		// Square
		Gui.drawRect(width * 4 / 10 - (int) (itemXSize * 2) + (int) (itemXSize / 4),
				height / 15 + (int) itemYSize + (int) (itemYSize / 4),
				width * 4 / 10 - (int) (itemXSize * 2) + (int) (itemXSize / 4) + (int) itemXSize * 2,
				height / 15 + (int) itemYSize + (int) (itemYSize / 4) + (int) (itemYSize * 2), 0x01FFFFFF);
		...
		GlStateManager.popMatrix();
		// this.itemRender.renderItemOverlayIntoGUI(font, stack, width * 1 / 2 - 140,
		// 30, "1");
		this.zLevel = 0.0F;
		this.itemRender.zLevel = 0.0F;
